...
use crate::elf::{parse_elf, Elf, ELF_HDR_SIZE};
...
fn _debug_print_stack(stack: &Vec<u8>, base_addr: usize) {
...
pub fn load_bochs() -> Result<Bochs, LucidErr> {
    // Sanity checks
    assert!(PAGE_SIZE == 0x1000);
    assert!(LOAD_TARGET % PAGE_SIZE == 0);
    assert!(GUARD_SIZE % PAGE_SIZE == 0);
    assert!(STACK_ALLOC_SIZE % PAGE_SIZE == 0);
    assert!(STACK_DATA_MAX % PAGE_SIZE == 0);
    ...
    let bochs = Bochs {
        entry: bochs_addr as u64 + elf.elf_header.entry,
